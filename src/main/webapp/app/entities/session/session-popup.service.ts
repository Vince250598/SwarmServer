        ...
        return new Promise<NgbModalRef>((resolve, reject) => {
            ...
            if (id) {
                this.sessionService.find(id).subscribe((session) => {
                    session.started = this.datePipe
                        .transform(session.started, 'yyyy-MM-ddTHH:mm:ss');
                    session.finished = this.datePipe
                        .transform(session.finished, 'yyyy-MM-ddTHH:mm:ss');
